class ValorIncorrectoEx(Exception):
    ...
    def __init__(self):
        super().__init__("[ERROR] El valor ingresado no puede ser negativo")

class HoraIncorrectaEx(Exception):

    def __init__(self):
        super().__init__("[ERROR] La hora no puede ser menor que cero ni mayor que 23")
...
def ejercicio2():
    ...
    if nota_uno < 0 or nota_dos < 0 or nota_tres <0:
...
def ejercicio11():
    ...
    if edad < 0 or ingreso_mensual < 0 or score_crediticio < 0:
        raise ValorIncorrectoEx()
    condicion = 18 <= edad <= 65 and ingreso_mensual >= 300000 and score_crediticio >= 700
    situacion = "Si" if condicion else "No"
...
def ejercicio12():
    ...
    hora_transaccion = int(input("Ingrese la hora en que se realizo la transaccion [0-23]: "))
    if monto < 0:
        raise ValorIncorrectoEx()
    if 0 > hora_transaccion > 23:
        raise HoraIncorrectaEx()
...
def ejercicio17():
    ...
    if n < 0:
        ...
        raise Exception("[ERROR] El valor ingresado no puede ser nulo")
    suma = 0
    for i in range(n):
        edad = int(input("Ingrese una edad: "))
        suma = suma + edad
